...
class TestRWLock(unittest.TestCase):
    def execute(self, what: Callable[[], None]) -> None:
        def runnit() -> None:
            for _ in range(5):
                what()

        threads: list[threading.Thread] = [
            threading.Thread(target=runnit) for _ in range(10)
        ]
        for t in threads:
            t.start()
        for t in threads:
    ...
    def test_simple_read_lock(self) -> None:
        lock: RWLock = RWLock()
        lock.lock_read()
        time.sleep(0.01)
        lock.unlock_read()
    ...
    def test_simple_write_lock(self) -> None:
        lock: RWLock = RWLock()
        lock.lock_write()
        time.sleep(0.01)
        lock.unlock_write()
    ...
    def test_lock_write(self) -> None:
        lock: RWLock = RWLock()
        count: AtomicInt64 = AtomicInt64()
        ok: AtomicFlag = AtomicFlag(True)
        ran: AtomicFlag = AtomicFlag(False)

        def check_serialized() -> None:
            try:
                lock.lock_write()
                count.incr()
                if count > 1:
    ...
    def test_lock_read(self) -> None:
        lock: RWLock = RWLock()
        count: AtomicInt64 = AtomicInt64()
        ok: AtomicFlag = AtomicFlag(False)
        ran: AtomicFlag = AtomicFlag(False)

        def check_notserialized() -> None:
            try:
                lock.lock_read()
                count.incr()
                if count > 1:
    ...
    def test_lock_read_write(self) -> None:
        lock: RWLock = RWLock()
        rcount: AtomicInt64 = AtomicInt64()
        wcount: AtomicInt64 = AtomicInt64()
        rok: AtomicFlag = AtomicFlag(False)
        wokr: AtomicFlag = AtomicFlag(True)
        wokw: AtomicFlag = AtomicFlag(True)
        rran: AtomicFlag = AtomicFlag(False)
    ...
    def test_readers(self) -> None:
        ...
        threads: list[threading.Thread] = [
            threading.Thread(target=read_wait) for _ in range(10)
        ]
    ...
    def test_writers_block_readers(self) -> None:
        ...
        new_start: AtomicInt64 = AtomicInt64()
        ...
        # Get 5 read locks
        threads: list[threading.Thread] = [
            threading.Thread(target=read_wait1) for _ in range(5)
        ]
        ...
        new_threads: list[threading.Thread] = [
            threading.Thread(target=read_wait2) for _ in range(5)
        ]
        ...
        self.assertFalse(is_writer_locked)
        self.assertEqual(lock.readers(), 0)

    def test_writers_waiting(self) -> None:
        lock: RWLock = RWLock()
        done: AtomicFlag = AtomicFlag(False)
        started: AtomicFlag = AtomicFlag(False)
        locked: AtomicFlag = AtomicFlag(False)
...
class TestRWLockSignals(unittest.TestCase):
    def test_interrupt_handling_write(self) -> None:
        def acquire(lock: object) -> None:
            lock.lock_write()  # pyre-ignore[16]

        def release(lock: object) -> None:
            lock.unlock_write()  # pyre-ignore[16]

        run_interrupt_handling(self, RWLock(), acquire, release)

    def test_interrupt_handling_read(self) -> None:
        phase: AtomicInt64 = AtomicInt64(0)

        def acquire(lock: object) -> None:
            if phase == 0:
                lock.lock_write()  # pyre-ignore[16]
            elif phase == 1:
                lock.lock_read()  # pyre-ignore[16]
            else:
        ...
        def release(lock: object) -> None:
            if phase == 1:
                lock.unlock_write()  # pyre-ignore[16]
            if phase == 2:
                lock.unlock_read()  # pyre-ignore[16]
            else:
